...
public class App {
    ...
    //HERENCIA USUARIO ESTUDIANTE
        Estudiante estudiante = new Estudiante("Diego","Rincon Alvarez", 1115189532,
                "diego.rincona@uqvirtual.edu.co", "CLL50", 318862555, 32,74,
                1.68,LocalDate.of(1992,11,12),RutinasDeFuerza.BICEPS,
                "programacion","Ingenieria de Sistemas",2);


    //HERENCIA USUARIO TRABAJADOR UQ
        Trabajadoresuq trabajadoresuq = new Trabajadoresuq("Juan", "Jimenez", 289643,
                "Cerrito", "juan.j@uqvirtual.edu.co",32122222,20,LocalDate.of(2005,
                4,15), RutinasDeFuerza.PESO_MUERTO,"Aarea de sistemas seguridad " +
                "de la informacion", 289643, "jefe seguridad de la informacion d ela universidad" +
                " del quindio",87,1.90);


    //HERENCIA USUARIO EXTERNOS
        Externos externos = new Externos("Samuel", "alvarado", 108565432, "circasia",
                "samuel.1@gmail.com", 321324321, 23,89,1.60, LocalDate.of(2002,
                12,26),RutinasDeFuerza.SENTADILLAS, "maestro de construccion",
                "constructor obra negra","avenida centenario");
    ...
    //INGRESAR ENTRENADORES
        Entrenadores entrenadores = new Entrenadores("Juan", "peñas",59065432,
                316666666,"Cerrito", "Entrenador","ACTIVO","YOGA",estudiante,
                EstadoDeEntrenadores.LABORANDO);



    //SISTEMA DE IMPRESION
        System.out.println("Los Datos Del Estudiante Inscrito Son:"+estudiante.getNombres()+estudiante.getApellidos()
                +estudiante.getIdentificacion() +estudiante.getDireccion()+estudiante.getCorreo()
                +estudiante.getTelefono()+estudiante.getEdad()+estudiante.getFechaNacimiento()
                +estudiante.getRutinasDeFuerza()+estudiante.getCursoAcademico()+estudiante.getPrograma()
                +estudiante.getSemestre()+ "la masa corporal deL estudiante es de:"+estudiante.calcularMedidas());

        System.out.println("Los Datos Persomales Del Usuario Quien Labora En La Universidad Del Quindio Son:"
                +trabajadoresuq.getNombres() +trabajadoresuq.getApellidos()+trabajadoresuq.getIdentificacion()
                +trabajadoresuq.getDireccion()+trabajadoresuq.getCorreo() +trabajadoresuq.getTelefono()
                +trabajadoresuq.getEdad()+trabajadoresuq.getFechaNacimiento()+trabajadoresuq.getRutinasDeFuerza()
                +trabajadoresuq.getLugarDondeLabora()+trabajadoresuq.getCodigoServicio()+trabajadoresuq.getCargo()
                +"la masa corporal del Empleado es de:" +trabajadoresuq.calcularMedidas());

        System.out.println("Los datos del usuario Externo son:"+externos.getNombres()+externos.getApellidos()
                +estudiante.getIdentificacion() +externos.getDireccion()+externos.getCorreo()+externos.getTelefono()
                +externos.getEdad()+externos.getFechaNacimiento() +externos.getRutinasDeFuerza()+externos.getCargo()
                +externos.getOcupacion()+externos.getEmpresaDondeLabora()+ "la masa corporal del Usuario Externo es de:"
        ...
        System.out.println("Bienvenido. Registra las clases en las que quieras participar"+clases.getNombre()
                +clases.getApellido()+clases.getIdentificacion() +clases.getFechaInicio()+clases.getFechaFinal()
                +clases.getHoraInicio()+clases.getHoraFinal());

        System.out.println(" La Actividad Deportiva de hoy es la siguiente"+actividadDeportiva.getNombre()
                +actividadDeportiva.getApellido() +actividadDeportiva.getIdentificacion()
                +actividadDeportiva.getTelefono()+actividadDeportiva.getRutinasDeFuerza()
                +actividadDeportiva.getActividadCardiovascular()+actividadDeportiva.getMaquinas());
